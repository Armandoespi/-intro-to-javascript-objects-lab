...
// Add a new property "difficulty" to the existing game object
...
const additionalPokemon = [
    pokemon.find(p => p.name === "Charmander"), // Fire type
    pokemon.find(p => p.name === "Pikachu"),    // Electric type
    pokemon.find(p => p.name === "Eevee")       // Normal type
  ];
...
// Evolving the starter Pokémon in the party

// Function to evolve a specific starter Pokémon
function evolvePokemon(pokemon) {
    ...
    return evolvedPokemon[pokemon.number] || pokemon;
}

// Loop through the party to evolve starter Pokémon
game.party.forEach((pokemon, index) => {
    if (pokemon.starter === true) {
        const evolved = evolvePokemon(pokemon);
        // Replace the old starter with the evolved Pokémon using splice
...
// Assuming game.party contains Pokémon objects
